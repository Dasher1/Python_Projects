...
#from resources.bs4 import BeautifulStoneSoup, BeautifulSoup
#from bs4 import BeautifulStoneSoup, BeautifulSoup
...
def addSource(url=None):
        ...
        if media_info:
            ...
            if '\\' in source_url:
                nameStr = source_url.split('\\')[-1].split('.')[0]
            if '%' in nameStr:
                nameStr = urllib.parse.unquote_plus(nameStr)
...
# def get_xml_database(url, browse=False):
        ...
        # soup=BeautifulSoup(makeRequest(url),features="xml")
...
#                        print 'found m3u data'
        ...
        ###Python3 ERROR
        #print("SOUR:",data,type(data))
        #import lxml
        #######################################
        res=BeautifulSoup(data,features="xml")
        #res=BeautifulSoup(data,features="lxml")
        #res=BeautifulSOAP(data, convertEntities=BeautifulStoneSoup.XML_ENTITIES)
        print("SOUR:",str(res),type(res))
        return res
    ...
    # for other,channel_name,stream_url in match:
        ...
        # if 'type' in other:
            ...
            # elif mode_type == 'regex':
                ...
                # continue
            # elif mode_type == 'ftv':
                # stream_url = 'plugin://plugin.video.F.T.V/?name='+urllib.parse.quote(channel_name) +'&url=' +stream_url +'&mode=125&ch_fanart=na'
        # elif tsdownloader and '.ts' in stream_url:
            # stream_url = 'plugin://plugin.video.f4mTester/?url='+urllib.parse.quote_plus(stream_url)+'&amp;streamtype=TSDOWNLOADER&name='+urllib.parse.quote(channel_name)
        # elif hlsretry and '.m3u8' in stream_url:
            # stream_url = 'plugin://plugin.video.f4mTester/?url='+urllib.parse.quote_plus(stream_url)+'&amp;streamtype=HLSRETRY&name='+urllib.parse.quote(channel_name)
            ...
            try:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('utube'):
                        if not i.string == None:
                            if ' ' in i.string :
                                utube = 'plugin://plugin.video.youtube/search/?q='+ urllib.parse.quote_plus(i.string)
                                isJsonrpc=utube
...
#                        print 'urllib2.getproxies',urllib2.getproxies()
                        current_proxies=urllib.request.ProxyHandler(urllib2.getproxies())
...
#                            urllib2.getproxies= lambda: {}
                            if pageUrl[:5]=="https":
                                proxy = urllib.request.ProxyHandler({ 'https' : proxytouse})
                                #req.set_proxy(proxytouse, 'https')
                            else:
                                proxy = urllib.request.ProxyHandler({ 'http'  : proxytouse})
...
#                            print 'appending cookie to cookiejar',m['appendcookie']
                            ...
                            for h in cookiestoApend:
                                n,v=h.split('=')
                                w,n= n.split(':')
                                ck = http.cookiejar.Cookie(version=0, name=n, value=v, port=None, port_specified=False, domain=w, domain_specified=False, domain_initial_dot=False, path='/', path_specified=True, secure=False, expires=None, discard=True, comment=None, comment_url=None, rest={'HttpOnly': None}, rfc2109=False)
...
#                            print 'noredirect','noredirect' in m
                            ...
                            if 'noredirect' in m:
                                opener = urllib.request.build_opener(cookie_handler,NoRedirection, urllib.request.HTTPBasicAuthHandler(), urllib.request.HTTPHandler())
                                opener = urllib.request.install_opener(opener)
                        elif 'noredirect' in m:
                            opener = urllib.request.build_opener(NoRedirection, urllib.request.HTTPBasicAuthHandler(), urllib.request.HTTPHandler())
                            opener = urllib.request.install_opener(opener)
...
#                            print '..........................connection//////.',m['connection']
                        ...
                        try:
                            ...
                            if 'proxy' in m and not current_proxies is None:
                                urllib.request.install_opener(urllib.request.build_opener(current_proxies))
...
#                            print 'rawpost'
                        ...
                        if 'htmlunescape' in m:
                            ...
                            import html.parser
                            val=html.parser.HTMLParser().unescape(val)
                        try:
                            url = url.replace("$doregex[" + k + "]", val)
                        except: url = url.replace("$doregex[" + k + "]", val.decode("utf-8"))
                        #print 'ur',url
...
def get_packed_iphonetv_url(page_data):
    import re,base64,urllib.request,urllib.parse,urllib.error;
    s=page_data
    while 'geh(' in s:
...
#        print 'captcha_url',captcha_url
    ...
    if captcha_reload_response_chall:
        if type==1:
            return 'recaptcha_challenge_field='+urllib.parse.quote_plus(captcha_reload_response_chall)+'&recaptcha_response_field='+urllib.parse.quote_plus(solution)
        elif type==2:
...
def get_decode(str,reg=None):
    ...
    s1 = urllib.parse.unquote(str[0: len(str)-1]);
    t = '';
...
#    print 'js',js
    if (not js==None) and len(js)>0:
        for j in js:
            ...
            str=str.replace(j ,urllib.parse.unquote(j))
    return str
        ...
        if '$$LSPlayOnlyOne$$' in mu_playlist[0]:
            ...
            for i in mu_playlist:
                ...
                if '$$lsname=' in i:
                    ...
                    d_name=urllib.parse.urlparse(i).netloc
                    if d_name == '':
                        names.append(name)
                    else:
                        names.append(d_name)
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if regexs and len(regexs)>0:
            u=sys.argv[0]+"?url="+urllib.parse.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.parse.quote_plus(name)+"&fanart="+urllib.parse.quote_plus(fanart)+"&regexs="+regexs
        else:
...
#            print 'parentalblockedpin',parentalblockedpin
            if len(parentalblockedpin)>0:
                if parentalblock:
                    contextMenu.append(('Disable Parental Block','XBMC.RunPlugin(%s?mode=55&name=%s)' %(sys.argv[0], urllib.parse.quote_plus(name))))
                else:
                    contextMenu.append(('Enable Parental Block','XBMC.RunPlugin(%s?mode=56&name=%s)' %(sys.argv[0], urllib.parse.quote_plus(name))))
            ...
            if showcontext == '!!update':
                fav_params2 = (
                    ...
                    %(sys.argv[0], urllib.parse.quote_plus(reg_url), regexs)
                    )
...
def uni(string, encoding = 'utf-8'):
    if isinstance(string, str):
        if not isinstance(string, str):
            string = str(string, encoding, 'ignore')
    return string
...
def pluginquerybyJSON(url,give_me_result=None,playlist=False):
    ...
    if 'error' in json_folder_detail:
        ...
        for i in json_folder_detail['result']['files'] :
            ...
            meta = dict((k,v) for k, v in i.items() if not v == '0' or not v == -1 or v == '')
            meta.pop("file", None)
            if i['filetype'] == 'file':
                if playlist:
...
#                print 'setting as folder in link'
        ...
        if 'plugin://plugin.video.youtube/play/?video_id=' in url:
              ...
              contextMenu.append(('!!Download [COLOR blue]Audio!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=24&name=%s)'
                                      %(sys.argv[0], urllib.parse.quote_plus(yt_audio_url), urllib.parse.quote_plus(name))))
        u=sys.argv[0]+"?"
        play_list = False
        if playlist:
        ...
        if (not play_list) and not any(x in url for x in g_ignoreSetResolved) and not '$PLAYERPROXY$=' in url:#  (not url.startswith('plugin://plugin.video.f4mTester')):
            if regexs:
                ...
                if '$pyFunction:playmedia(' not in urllib.parse.unquote_plus(regexs) and 'notplayable' not in urllib.parse.unquote_plus(regexs) and 'listrepeat' not in  urllib.parse.unquote_plus(regexs) :
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                ...
                try:
                    ...
                    fav_params = (
                        '%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                        %(sys.argv[0], urllib.parse.quote_plus(name), urllib.parse.quote_plus(url), urllib.parse.quote_plus(iconimage.encode("utf-8")), urllib.parse.quote_plus(fanart.encode("utf-8")))
                        )
                if playlist:
...
playitem=''
try:
    playitem=urllib.parse.unquote_plus(params["playitem"])
except:
    pass
...
#        print 'Not setting setResolvedUrl'
    ...
    data=None
    if regexs and 'listrepeat' in urllib.parse.unquote_plus(regexs):
...
#        print 'regexs',regexs
 ...
 #       print 'final regexs',regexs,regexname
    ...
    #                print 'i is ',i, len(obj), len(newcopy)
                    if len(newcopy)>0:
                        for the_keyO, the_valueO in newcopy.items():
                            if the_valueO is not None:
                                for the_key, the_value in the_valueO.items():
        ...
        #                                print repr(obj[i])
                                        if type(the_value) is dict:
                                            for the_keyl, the_valuel in the_value.items():
